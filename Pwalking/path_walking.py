...
def get_all_valids_paths(plant_img):
    ...
    #create a dictionary to store the paths foreach tip
    best_paths = [TipPathsInfo(tip) for tip in tips]
        ...
        while(not end):
            ...
            if len(source_found)>0:
                ...
                if len(stashed_tip_paths_items) == 0 :
                    ...
                    tip_root_path = get_first_stashed_path_info()
            else:
                if len(neighbors) == 0:
                    print("no neighbors")
                    #if there aren't any stashed paths, return
                    ...
                    stash_path_info(tip_root_path,neighbors[1])
                    set_next_node_info(tip_root_path, neighbors[0])
...
def stash_path_info(tip_root_path,next_neigh_node):
...
def get_first_stashed_path_info():
